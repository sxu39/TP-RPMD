...
inline void _DP_DeepPotComputeadd<double>(DP_DeepPot *dp,
  ...
  DP_DeepPotCompute2add(dp, nframes, natom, coord, atype, cell, fparam, aparam,
                     energy, force, virial, dE_dN, atomic_energy, atomic_virial);
}
...
inline void _DP_DeepPotComputeNListadd<double>(DP_DeepPot *dp,
                                            ...
                                            double *atomic_energy,
                                            double *atomic_virial) {
  DP_DeepPotComputeNList2add(dp, nframes, natom, coord, atype, cell, nghost, nlist,
                          ago, fparam, aparam, energy, force, virial, dE_dN,
                          atomic_energy, atomic_virial);
}
...
inline void _DP_DeepPotComputeNListadd<float>(DP_DeepPot *dp,
  ...
  DP_DeepPotComputeNListf2add(dp, nframes, natom, coord, atype, cell, nghost,
                           nlist, ago, fparam, aparam, energy, force, virial, dE_dN,
                           atomic_energy, atomic_virial);
}
...
template <typename FPTYPE>  //?
inline void _DP_DeepPotComputeMixedTypeadd(DP_DeepPot *dp,
                                        const int nframes,
                                        const int natom,
                                        const FPTYPE *coord,
                                        const int *atype,
...
template <>
inline void _DP_DeepPotComputeMixedTypeadd<double>(DP_DeepPot *dp,
                                                const int nframes,
                                                const int natom,
                                                const double *coord,
                                                const int *atype,
                                                ...
                                                double *atomic_energy,
                                                double *atomic_virial) {
  DP_DeepPotComputeMixedTypeadd(dp, nframes, natom, coord, atype, cell, fparam,
                             aparam, energy, force, virial, dE_dN, atomic_energy,
                             atomic_virial);
}

template <>
inline void _DP_DeepPotComputeMixedTypeadd<float>(DP_DeepPot *dp,
                                               const int nframes,
                                               const int natom,
                                               const float *coord,
                                               const int *atype,
                                               ...
                                               float *atomic_energy,
                                               float *atomic_virial) {
  DP_DeepPotComputeMixedTypefadd(dp, nframes, natom, coord, atype, cell, fparam,
                              aparam, energy, force, virial, dE_dN, atomic_energy,
                              atomic_virial);
}
...
template <typename FPTYPE> //?
inline void _DP_DeepPotModelDeviComputeNListadd(DP_DeepPotModelDevi *dp,
                                             const int natom,
                                             const FPTYPE *coord,
                                             const int *atype,
                                             const FPTYPE *cell,
...
template <>
inline void _DP_DeepPotModelDeviComputeNListadd<double>(DP_DeepPotModelDevi *dp,
                                                     const int natom,
                                                     const double *coord,
                                                     const int *atype,
                                                     const double *cell,
                                                     ...
                                                     double *atomic_energy,
                                                     double *atomic_virial) {
  DP_DeepPotModelDeviComputeNList2add(dp, 1, natom, coord, atype, cell, nghost,
                                   nlist, ago, fparam, aparam, energy, force,
                                   virial, dE_dN, atomic_energy, atomic_virial);
}

template <>
inline void _DP_DeepPotModelDeviComputeNListadd<float>(DP_DeepPotModelDevi *dp,
                                                    const int natom,
                                                    const float *coord,
                                                    const int *atype,
                                                    const float *cell,
                                                    ...
                                                    float *atomic_energy,
                                                    float *atomic_virial) {
  DP_DeepPotModelDeviComputeNListf2add(dp, 1, natom, coord, atype, cell, nghost,
                                    nlist, ago, fparam, aparam, energy, force,
                                    virial, dE_dN, atomic_energy, atomic_virial);
}
...
class DeepPot {
 public:
  ...
  void compute(
    ...
    _DP_DeepPotComputeadd<VALUETYPE>(dp, nframes, natoms, coord_, atype_, box_,
                                  fparam__, aparam__, ener_, force_, virial_, dE_dN_,
                                  nullptr, nullptr);
    DP_CHECK_OK(DP_DeepPotCheckOK, dp);
  };
  /**
  ...
  void compute(
    ...
    _DP_DeepPotComputeNListadd<VALUETYPE>(dp, nframes, natoms, coord_, atype_,
                                       box_, nghost, lmp_list.nl, ago, fparam__,
                                       aparam__, ener_, force_, virial_, dE_dN_,
                                       atomic_ener_, atomic_virial_);
    DP_CHECK_OK(DP_DeepPotCheckOK, dp);
  };
  /**
  ...
  void compute_mixed_type(
    ...
    _DP_DeepPotComputeMixedTypeadd<VALUETYPE>(dp, nframes, natoms, coord_, atype_,
                                           box_, fparam__, aparam__, ener_,
                                           force_, virial_, dE_dN_, nullptr, nullptr);
    DP_CHECK_OK(DP_DeepPotCheckOK, dp);
  };
  /**
   * @brief Evaluate the energy, force, virial, atomic energy, and atomic virial
  ...
  void compute_mixed_type(
    ...
    _DP_DeepPotComputeMixedTypeadd<VALUETYPE>(
        dp, nframes, natoms, coord_, atype_, box_, fparam__, aparam__, ener_,
        force_, virial_, dE_dN_, atomic_ener_, atomic_virial_);
    DP_CHECK_OK(DP_DeepPotCheckOK, dp);
  };
  /**
...
class DeepPotModelDevi {
 public:
  ...
  void compute(
    ...
    std::vector<VALUETYPE> force_flat(numb_models * natoms * 3);
    std::vector<VALUETYPE> virial_flat(numb_models * 9);
    std::vector<double> dE_dN_flat(numb_models);
    double *ener_ = &energy_flat[0];
    VALUETYPE *force_ = &force_flat[0];
    VALUETYPE *virial_ = &virial_flat[0];
    double *dE_dN_ = &dE_dN_flat[0];
    std::vector<VALUETYPE> fparam_, aparam_;
    validate_fparam_aparam(nframes, (aparam_nall ? natoms : (natoms - nghost)),
                           fparam, aparam);
    tile_fparam_aparam(fparam_, nframes, dfparam, fparam);
    ...
    _DP_DeepPotModelDeviComputeNListadd<VALUETYPE>(
        dp, natoms, coord_, atype_, box_, nghost, lmp_list.nl, ago, fparam__,
        aparam__, ener_, force_, virial_, dE_dN_, nullptr, nullptr);
    DP_CHECK_OK(DP_DeepPotModelDeviCheckOK, dp);
    ...
    force.resize(numb_models);
    virial.resize(numb_models);
    dE_dN.resize(numb_models);
    for (int i = 0; i < numb_models; i++) {
      ener[i] = energy_flat[i];
      force[i].resize(natoms * 3);
      virial[i].resize(9);
      dE_dN[i] = dE_dN_flat[i];
      for (int j = 0; j < natoms * 3; j++) {
        force[i][j] = force_flat[i * natoms * 3 + j];
      }
      for (int j = 0; j < 9; j++) {
  ...
  void compute(
    ...
    _DP_DeepPotModelDeviComputeNListadd<VALUETYPE>(
        dp, natoms, coord_, atype_, box_, nghost, lmp_list.nl, ago, fparam__,
        aparam__, ener_, force_, virial_, dE_dN_, atomic_ener_, atomic_virial_);
    DP_CHECK_OK(DP_DeepPotModelDeviCheckOK, dp);
