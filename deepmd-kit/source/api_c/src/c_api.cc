...
inline void DP_DeepPotCompute_variantadd(DP_DeepPot* dp,
  ...
  std::vector<double> e, dEdN;
  std::vector<VALUETYPE> f, v, ae, av;

  DP_REQUIRES_OK(dp, dp->dp.compute(e, f, v, dEdN, ae, av, coord_, atype_, cell_,
                                    fparam_, aparam_));
  // copy from C++ vectors to C arrays, if not NULL pointer
  if (energy) {
    std::copy(e.begin(), e.end(), energy);
  ...
  if (dE_dN) {
    std::copy(dEdN.begin(), dEdN.end(), dE_dN);
  }
  if (atomic_energy) {
    std::copy(ae.begin(), ae.end(), atomic_energy);
  }
...
template <typename VALUETYPE> //?
inline void DP_DeepPotComputeMixedType_variantadd(DP_DeepPot* dp,
                                               const int nframes,
                                               const int natoms,
                                               const VALUETYPE* coord,
                                               const int* atype,
  ...
  DP_REQUIRES_OK(
      dp, dp->dp.compute_mixed_type(e, f, v, dEdN, ae, av, nframes, coord_, atype_,
                                    cell_, fparam_, aparam_));
  // copy from C++ vectors to C arrays, if not NULL pointer
  if (energy) {
    std::copy(e.begin(), e.end(), energy);
...
template void DP_DeepPotComputeMixedType_variantadd<double>(DP_DeepPot* dp,
                                                         const int nframes,
                                                         const int natoms,
                                                         const double* coord,
                                                         const int* atype,
...
template void DP_DeepPotComputeMixedType_variantadd<float>(DP_DeepPot* dp,
                                                        const int nframes,
                                                        const int natoms,
                                                        const float* coord,
                                                        const int* atype,
...
template <typename VALUETYPE>
void DP_DeepPotModelDeviComputeNList_variantadd(DP_DeepPotModelDevi* dp,
                                             const int nframes,
                                             const int natoms,
                                             const VALUETYPE* coord,
                                             const int* atype,
...
template void DP_DeepPotModelDeviComputeNList_variantadd<double>(
    DP_DeepPotModelDevi* dp,
    const int nframes,
    const int natoms,
    const double* coord,
...
template void DP_DeepPotModelDeviComputeNList_variantadd<float>(
    DP_DeepPotModelDevi* dp,
    const int nframes,
    const int natoms,
    const float* coord,
...
void DP_DeepPotComputeadd(DP_DeepPot* dp,
                       const int natoms,
                       const double* coord,
                       const int* atype,
                       const double* cell,
                       ...
                       double* atomic_energy,
                       double* atomic_virial) {
  DP_DeepPotCompute_variantadd<double>(dp, 1, natoms, coord, atype, cell, NULL,
                                    NULL, energy, force, virial, dE_dN, atomic_energy,
                                    atomic_virial);
}

void DP_DeepPotComputefadd(DP_DeepPot* dp,
                        const int natoms,
                        const float* coord,
                        const int* atype,
                        const float* cell,
                        ...
                        float* atomic_energy,
                        float* atomic_virial) {
  DP_DeepPotCompute_variantadd<float>(dp, 1, natoms, coord, atype, cell, NULL,
                                   NULL, energy, force, virial, dE_dN, atomic_energy,
                                   atomic_virial);
}

void DP_DeepPotComputeNListadd(DP_DeepPot* dp,
                            const int natoms,
                            const double* coord,
                            const int* atype,
                            const double* cell,
  ...
  DP_DeepPotComputeNList_variantadd<double>(
      dp, 1, natoms, coord, atype, cell, nghost, nlist, ago, NULL, NULL, energy,
      force, virial, dE_dN, atomic_energy, atomic_virial);
}

void DP_DeepPotComputeNListfadd(DP_DeepPot* dp,
                             const int natoms,
                             const float* coord,
                             const int* atype,
                             const float* cell,
...
void DP_DeepPotCompute2add(DP_DeepPot* dp,
  ...
  DP_DeepPotCompute_variantadd<double>(dp, nframes, natoms, coord, atype, cell,
                                    fparam, aparam, energy, force, virial, dE_dN,
                                    atomic_energy, atomic_virial);
}
...
void DP_DeepPotComputeMixedTypeadd(DP_DeepPot* dp,
                                const int nframes,
                                const int natoms,
                                const double* coord,
                                const int* atype,
                                ...
                                double* atomic_energy,
                                double* atomic_virial) {
  DP_DeepPotComputeMixedType_variantadd<double>(
      dp, nframes, natoms, coord, atype, cell, fparam, aparam, energy, force,
      virial, dE_dN, atomic_energy, atomic_virial);
}

void DP_DeepPotComputeMixedTypefadd(DP_DeepPot* dp,
                                 const int nframes,
                                 const int natoms,
                                 const float* coord,
                                 const int* atype,
                                 ...
                                 float* atomic_energy,
                                 float* atomic_virial) {
  DP_DeepPotComputeMixedType_variantadd<float>(
      dp, nframes, natoms, coord, atype, cell, fparam, aparam, energy, force,
      virial, dE_dN, atomic_energy, atomic_virial);
}
...
void DP_DeepPotModelDeviComputeNList2add(DP_DeepPotModelDevi* dp,
                                      const int nframes,
                                      const int natoms,
                                      const double* coord,
                                      const int* atype,
                                      ...
                                      double* atomic_energy,
                                      double* atomic_virial) {
  DP_DeepPotModelDeviComputeNList_variantadd<double>(
      dp, nframes, natoms, coord, atype, cell, nghost, nlist, ago, fparam,
      aparam, energy, force, virial, dE_dN, atomic_energy, atomic_virial);
}

void DP_DeepPotModelDeviComputeNListf2add(DP_DeepPotModelDevi* dp,
                                       const int nframes,
                                       const int natoms,
                                       const float* coord,
                                       const int* atype,
                                       ...
                                       float* atomic_energy,
                                       float* atomic_virial) {
  DP_DeepPotModelDeviComputeNList_variantadd<float>(
      dp, nframes, natoms, coord, atype, cell, nghost, nlist, ago, fparam,
      aparam, energy, force, virial, dE_dN, atomic_energy, atomic_virial);
}
