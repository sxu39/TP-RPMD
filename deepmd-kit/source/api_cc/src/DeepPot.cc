...
static void run_model(
  ...
  std::vector<Tensor> output_tensors;
  check_status(session->Run(
      input_tensors, {"o_energy", "o_force", "o_dE_dN", "o_atom_energy", "o_atom_virial"},
      {}, &output_tensors));
...
static void run_model(
  ...
  assert(nframes == 1);
  std::vector<ENERGYTYPE> dener_(1);
  std::vector<ENERGYTYPE> ddE_dN_(1);
  // call multi-frame version
  run_model<MODELTYPE, VALUETYPE>(dener_, dforce_, dvirial, ddE_dN_, session,
                                  input_tensors, atommap, nframes, nghost);
  dener = dener_[0];
  ddE_dN = ddE_dN_[0];
}
...
static void run_model(
  ...
  std::vector<ENERGYTYPE> ddE_dN_(1);
  // call multi-frame version
  run_model<MODELTYPE, VALUETYPE>(dener_, dforce_, dvirial, ddE_dN_, datom_energy_,
                                  datom_virial_, session, input_tensors,
                                  atommap, nframes, nghost);
  dener = dener_[0];
  ddE_dN = ddE_dN_[0];
...
void DeepPot::compute(ENERGYVTYPE& dener,
  ...
  if (dtype == tensorflow::DT_DOUBLE) {
    int ret = session_input_tensors<double>(input_tensors, dcoord_, ntypes,
                                            ...
                                            aparam, atommap, "", aparam_nall);
    assert(ret == nloc);
    run_model<double>(dener, dforce_, dvirial, ddE_dN, session, input_tensors, atommap,
                      nframes);
  } else {
    int ret = session_input_tensors<float>(input_tensors, dcoord_, ntypes,
                                           datype_, dbox, cell_size, fparam,
                                           aparam, atommap, "", aparam_nall);
    assert(ret == nloc);
    run_model<float>(dener, dforce_, dvirial, ddE_dN, session, input_tensors, atommap,
                     nframes);
  }
}
...
void DeepPot::compute(ENERGYVTYPE& dener,
  ...
  compute_inner(dener, dforce, dvirial, ddE_dN, dcoord, datype, dbox, nghost_real, ago,
                fparam, aparam);
  // bkw map
  dforce_.resize(nframes * fwd_map.size() * 3);
  select_map<VALUETYPE>(dforce_, dforce, bkw_map, 3, nframes, fwd_map.size(),
...
void DeepPot::compute(ENERGYVTYPE& dener,
  ...
  if (dtype == tensorflow::DT_DOUBLE) {
    int nloc = session_input_tensors<double>(input_tensors, dcoord_, ntypes,
                                             datype_, dbox, cell_size, fparam,
                                             aparam, atommap, "", aparam_nall);
    run_model<double>(dener, dforce_, dvirial, ddE_dN, datom_energy_, datom_virial_,
                      session, input_tensors, atommap, nframes);
  } else {
    int nloc = session_input_tensors<float>(input_tensors, dcoord_, ntypes,
                                            datype_, dbox, cell_size, fparam,
                                            aparam, atommap, "", aparam_nall);
    run_model<float>(dener, dforce_, dvirial, ddE_dN, datom_energy_, datom_virial_,
                     session, input_tensors, atommap, nframes);
  }
}
...
void DeepPot::compute(ENERGYVTYPE& dener,
  ...
  if (dtype == tensorflow::DT_DOUBLE) {
    int ret = session_input_tensors<double>(
        ...
        atommap, nghost_real, ago, "", aparam_nall);
    assert(nloc_real == ret);
    run_model<double>(dener, dforce, dvirial, ddE_dN, datom_energy, datom_virial,
                      session, input_tensors, atommap, nframes, nghost_real);
  } else {
    int ret = session_input_tensors<float>(
        input_tensors, dcoord, ntypes, datype, dbox, nlist, fparam, aparam,
        atommap, nghost_real, ago, "", aparam_nall);
    assert(nloc_real == ret);
    run_model<float>(dener, dforce, dvirial, ddE_dN, datom_energy, datom_virial,
                     session, input_tensors, atommap, nframes, nghost_real);
  }
...
void DeepPotModelDevi::compute(std::vector<ENERGYTYPE>& all_energy,
                               std::vector<std::vector<VALUETYPE>>& all_force,
                               std::vector<std::vector<VALUETYPE>>& all_virial,
                               std::vector<ENERGYTYPE>& all_dE_dN,
                               const std::vector<VALUETYPE>& dcoord_,
                               const std::vector<int>& datype_,
                               const std::vector<VALUETYPE>& dbox,
                               const int nghost,
  ...
  all_force.resize(numb_models);
  all_virial.resize(numb_models);
  all_dE_dN.resize(numb_models);
  assert(nloc == ret);
  for (unsigned ii = 0; ii < numb_models; ++ii) {
    std::vector<VALUETYPE> dforce;
    if (dtype == tensorflow::DT_DOUBLE) {
      run_model<double>(all_energy[ii], dforce, all_virial[ii], all_dE_dN[ii], sessions[ii],
                        input_tensors, atommap, 1, nghost_real);
    } else {
      run_model<float>(all_energy[ii], dforce, all_virial[ii], all_dE_dN[ii], sessions[ii],
                       input_tensors, atommap, 1, nghost_real);
    }
    // bkw map
    all_force[ii].resize(nframes * fwd_map.size() * 3);
...
void DeepPotModelDevi::compute(
  ...
  for (unsigned ii = 0; ii < numb_models; ++ii) {
    std::vector<VALUETYPE> dforce, datom_energy, datom_virial;
    if (dtype == tensorflow::DT_DOUBLE) {
      run_model<double>(all_energy[ii], dforce, all_virial[ii], all_dE_dN[ii], datom_energy,
                        datom_virial, sessions[ii], input_tensors, atommap, 1,
                        nghost_real);
    } else {
      run_model<float>(all_energy[ii], dforce, all_virial[ii], all_dE_dN[ii], datom_energy,
                       datom_virial, sessions[ii], input_tensors, atommap, 1,
                       nghost_real);
    }
    // bkw map
...
template <typename VALUETYPE> //?修改
void DeepPotModelDevi::compute_avg(VALUETYPE& dener,
                                   VALUETYPE& ddE_dN,
                                   const std::vector<VALUETYPE>& all_energy,
                                   const std::vector<VALUETYPE>& all_dE_dN) {
  assert(all_energy.size() == numb_models);
  assert(all_dE_dN.size() == numb_models);
  if (numb_models == 0) {
    return;
  }
  ...
  dener /= (VALUETYPE)(numb_models);
  ddE_dN /= (VALUETYPE)(numb_models);
}

template void DeepPotModelDevi::compute_avg<double>(
    double& dener, double& ddE_dN, 
    const std::vector<double>& all_energy,
    const std::vector<double>& all_dE_dN);

template void DeepPotModelDevi::compute_avg<float>(
    float& dener, float& ddE_dN, 
    const std::vector<float>& all_energy,
    const std::vector<float>& all_dE_dN);
