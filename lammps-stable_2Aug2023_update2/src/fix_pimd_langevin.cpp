...
void FixPIMDLangevin::nhc_mu_integrate() //?
{
  ...
  if (method == TPRPMD && np!=1 && universe->iworld == 0) //?TPRPMD
    ...
    if (eta_mass[0] > 0.0)
      eta_dotdot[0] = (kecurrent + inverse_np*0.5*(*Ne_mass)*(*Ne_dot)*(*Ne_dot) - np*ke_target)/eta_mass[0];
      // eta_dotdot[0] = (kecurrent + 0.5*(*Ne_mass)*(*Ne_dot)*(*Ne_dot) - np*ke_target)/eta_mass[0];
    else eta_dotdot[0] = 0.0;
  }
